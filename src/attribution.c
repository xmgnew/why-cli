...
WhyAttribution *why_attribute(const WhyHistory *h, const WhyCpuIncident *event,
    ...
    if (!first || !last || hz <= 0 ||
        (!event && mid(&last->frame.system) <= mid(&first->frame.system)) ||
        !capacity || capacity > WHY_ATTRIBUTION_CAPACITY ||
    ...
    r->start_ns = event ? event->start_ns : mid(&first->frame.system);
    r->end_ns = event ? event->end_ns : mid(&last->frame.system);
